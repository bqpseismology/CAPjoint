...
# Specify the folder of CRUST2 below
  basepath=$(pwd)
  CRUST=$basepath"/cmds/crust2/" 
  echo $CRUST

# dep1: upper limit; dep2: lower limit; depg: depth grid space
  dep1=5;  	     dep2=30; 		depg=5;
...
# P/SH switch: P only:-1 ; SH only: 1 ; P & SH: 0 (default)
  w_TEL_LOC=200;          w_P_SH=1;             s_P_SH=0;
...
echo -e "perl ./cmds/cap_3.pl -D1/0/0 -T60/60 -V-1/-1/-1 -G./greenFuncDir  -O -P1e-6/90/k   -H0.05 -L"$dur"  -Mvmodel_\$1/"$mag" -W1 -R0/360/0/90/-180/180  -S5/10/0 -C"$freq"  -I10/0.1  data  -Zweight.dat -A"$w_TEL_LOC"/"$w_P_SH"/"$s_P_SH" " > cap_3.cmd
...
echo -e "perl ./cmds/cap_3.pl -D1/0/0 -T60/60 -V-1/-1/-1 -G./greenFuncDir  -O -P1e-6/90/k   -H0.05 -L"$dur"  -Mvmodel_\$1/"$mag_revised" -W1 -R"$stk_min"/"$stk_max"/"$dip_min"/"$dip_max"/"$rak_min"/"$rak_max"  -S5/10/0 -C"$freq"  -I1/0.1  data  -Zweight.dat -A"$w_TEL_LOC"/"$w_P_SH"/"$s_P_SH" " > cap_3_rev.cmd

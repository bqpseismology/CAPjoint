...
#  `gmtset ANOT_MIN_ANGLE = 20`;
  `gmtset ANOT_FONT_SIZE_PRIMARY = 10p`;
  `gmtset ANOT_FONT_SIZE_SECONDARY = 10p`;
#  `gmtset ANOT_OFFSET = 0.2c`;
  `gmtset HEADER_FONT_SIZE = 18p`;
  `gmtset LABEL_FONT_SIZE = 12p`;
  `gmtset HEADER_OFFSET = 2.5c`;
...
#####################################################################################
  	$total_count=$loc_count+$tel_count ;
        $loc_line = $loc_count             ; # 控制绘图多少行,每行一个近震台站
	$tel_line = int($tel_count/2+0.5)  ; # 控制远震，每行两列
   	$total_line = $loc_line+$tel_line  ; # 总共多少行	
########################################################################################
  #	$width = 0.1*int(10*($tt+$t2+$sepa)/$sec_per_inch+0.5);
        $width = 7; # 单位？
        $hight = 7.5;

        $Rw=-$sepa*3; $Re=$tt+$sep+$sepa*2 ; $Rs=0; $Rn=$total_line+3;# 四个方向上的取值范围  
...
#================
	...
	$am1=100*$am; $am2=10*$am; # am2 become bigger and the waveform' amplitude become smaller 
        $shiftx=-$sepa;
...
#==loc
	...
	$pltSV  = " psbasemap   -JX${widthx}i/${hightx}i -R$Rwx/$Rex/$Rsx/$Rnx -Ba15g100:\"Rayleigh(V)\":/g1S  -K -P -O  -X${rightSF}i  >> $model.ps";
	$pltSR  = " psbasemap   -JX${widthx}i/${hightx}i -R$Rwx/$Rex/$Rsx/$Rnx -Ba15g100:\"Rayleigh(R)\":/g1S  -K -P -O  -X${rightSF}i  >> $model.ps";
...
#	$pltP2  = " psbasemap   -JX${width1}i/${hight1}i -R$Rw1/$Re1/$Rs1/$Rn1 -Ba15g100:.\"P\":/g1S  -K -P -O  -X${rightP2}i  >> $model.ps";
	...
	$pltP1  = " psbasemap   -JX${width1}i/${hight1}i -R$Rw1/$Re1/$Rs1/$Rn1 -Ba15g100:.\"P\":/g1S  -K -P -O  -X${shifttel}i -Y${shiftup}i >> $model.ps";
	$pltSH  = " psbasemap   -JX -R -Ba15g100:.\"SH\":/g1S -K -O -X${rightSH}i  >> $model.ps";
	$pltP2  = " psbasemap   -JX -R -Ba15g100:.\"P\":/g1S -K -O -X${rightP2}i  >> $model.ps";


	$plt00  = " psbasemap  -JX${width}i/${hight}i -R$Rw/$Re/$Rs/$Rn  -K -P -G255/255/255 -X4i -Y-3i > ${model}_1.ps";
#  	$plt1_loc  = "| $pssac -JX${width}i/${hight}i -R$Rw/$Re/$Rs/$Rn  -O -K -Ent-2 -M$am1/1 >> $model.ps";
#  	$plt1_tel  = "| $pssac -JX${width}i/${hight}i -R$Rw/$Re/$Rs/$Rn  -O -K -Ent-2 -M$am2/1 >> $model.ps";

  	$plt1_loc  = "| $pssac -JX${width}i/${hight}i -R$Rw/$Re/$Rs/$Rn  -O -K -Ent-2 -M0.5/ >> $model.ps";
  	$plt1_tel  = "| $pssac -JX${width}i/${hight}i -R$Rw/$Re/$Rs/$Rn  -O -K -Ent-2 -M0.5 >> $model.ps";
  	...
  	$plt3  = "| psmeca     -JX${width}i/${hight}i -R$Rw/$Re/$Rs/$Rn  -O -K -P -N -Sa${Sa}i  -G128/128/128  >> $model.ps";
  	$plt30  = "| psmeca    -JX${width}i/${hight}i -R$Rw/$Re/$Rs/$Rn  -O -K -P -N -Sa${Saa}i  -G128/128/128  >> ${model}_1.ps";
	$plt300  = "| psmeca   -JX${width}i/${hight}i -R$Rw/$Re/$Rs/$Rn  -O -K -P -N -Sa${Saa}i  -T0  >> ${model}_1.ps";
  	$plt4  = "| psxy       -JX${width}i/${hight}i -R$Rw/$Re/$Rs/$Rn  -O -K -P -N -Sc0.075i -W0.5p/255/255/0 -G255/0/0 >> $model.ps";
  	$plt41  = "| psxy       -JX${width}i/${hight}i -R$Rw/$Re/$Rs/$Rn  -O -K -P -N -St0.075i -W0.5p/255/255/0 -G255/0/0 >> $model.ps";
  	$plt40  = "| psxy      -JX${width}i/${hight}i -R$Rw/$Re/$Rs/$Rn  -O -K -P -N -Sc0.15i -W0.5p/255/255/0 -G255/0/0 >> ${model}_1.ps";
...
########################## above: set default seg and text positions ###################			
	...
	$intdist = int($stna_dist[1]/111.195+0.5);
					$pstext_string[$count_pstext++]="$pos_name_x $pos_az_y   8 0 0 CM $intaz\260 D($intdist\260)\n";     # on the left: az dist
					$pstext_string[$count_pstext++]="$pos_name_x $pos_name_y 10 0 16 CM $nt_na[0] $nt_na[1]\n";   # stna
#	$intdist = int($stna_dist[1]/111.195+0.5);
#					$pstext_string[$count_pstext++]="$pos_name_x $pos_dist_y  9 0 0 CM $intdist\271\n";   # dist
				}
                                if ($aa[5*$j+2]>0 )  { # weight is not zero
					$pssac_string2[$count_pssac2++]="$nam$com1 $str $pos_y 6/0/0/0\n";        # com1: data:black
 					$pssac_string2[$count_pssac2++]="$nam$com2 $str $pos_y 6/255/0/0\n";      # com2: green: red
					$pstext_string[$count_pstext++]="$str $pos_corr  10 0 0 RT $aa[5*$j+4]\n";  # cross correlation
					$pstext_string[$count_pstext++]="$str $pos_shift 8 0 2 RT $aa[5*$j+6]\n";  # shift second
                                } elsif ($keepBad>0) {
					$pssac_string2[$count_pssac2++]="$nam$com1 $str $pos_y 5/0/255/0\n";      # com1: data:green
 					$pssac_string2[$count_pssac2++]="$nam$com2 $str $pos_y 3/255/0/0\n";      # com2: green: red
...
=cut
	...
	$intdist = int($stna_dist[1]+0.5);
					$pstext_string[$count_pstext++]="$pos_name_x $pos_az_y   8 0 0 CM $intaz\260 $intdist km\n";     # loc: on the left -- az dist(in km)
					$pstext_string[$count_pstext++]="$pos_name_x $pos_name_y 10 0 16 CM $nt_na[0] $nt_na[1]\n";   
                                }
                                if ($aa[5*$j+2]>0 )  {  # this is the weight
					$pssac_string1[$count_pssac2++]="$nam$com1 $str $pos_y 6/0/0/0\n";        # com1: data:black
 					$pssac_string1[$count_pssac2++]="$nam$com2 $str $pos_y 6/255/0/0\n";      # com2: green: red
					$pstext_string[$count_pstext++]="$str $pos_corr  10 0 0 RT $aa[5*$j+4]\n";  # cross correlation
					$pstext_string[$count_pstext++]="$str $pos_shift 8 0 2 RT $aa[5*$j+6]\n";  # shift second

                              } elsif ($keepBad>0) {
					$pssac_string1[$count_pssac1++]="$nam$com1 $str $pos_y 6/0/255/0\n";      #com1: data:green
 					$pssac_string1[$count_pssac1++]="$nam$com2 $str $pos_y 6/255/0/0\n";      #com2: green: red
					$pstext_string[$count_pstext++]="$str $pos_corr 10 0 0 1 $aa[5*$j+4]\n";  # cross correlation
					$pstext_string[$count_pstext++]="$str $pos_shift 8 0 0 1 $aa[5*$j+6]\n";  # shift second
                                }
                                $offsetx+=$segment[$j];
